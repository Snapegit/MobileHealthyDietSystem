	...
	<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ff993310","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0px","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class=" select">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333333","textAlign":"right"}' class="title">发表日期</view>
				<picker :style='{"width":"100%","flex":"1","height":"auto"}' mode="date" :value="ruleForm.fabiaoriqi" @change="fabiaoriqiChange">
					<view :style='{"border":"2rpx solid #eeeeee","padding":"0 20rpx","color":"#666666","background":"#ffffff","width":"100%","lineHeight":"64rpx","fontSize":"28rpx"}' class="uni-input">{{ruleForm.fabiaoriqi?ruleForm.fabiaoriqi:"请选择发表日期"}}</view>
				</picker>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jiankangzhishi`, this.ruleForm.id);
		...
		methods: {
			...
			fabiaoriqiChange(e) {
				this.ruleForm.fabiaoriqi = e.target.value;
				this.$forceUpdate();
			},
...
//跨表计算判断
				...
				if((!this.ruleForm.fabiaoriqi)){
					this.$utils.msg(`发表日期不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`jiankangzhishi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`jiankangzhishi`, this.ruleForm);
						}else{
							await this.$api.add(`jiankangzhishi`, this.ruleForm);
						}
