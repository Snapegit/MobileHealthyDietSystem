...
import com.design.appproject.ui.binglidan.ListFilterDialog
...
import com.design.appproject.databinding.BinglidancommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.binglidan.ListModel
import com.design.appproject.ui.binglidan.ListAdapter
 ...
 * 病例单列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_BINGLIDAN)
class ListFragment : BaseBindingFragment<BinglidancommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("就诊时间","jiuzhenshijian")/*查询索引*/
    ...
    override fun initEvent() {
        setBarTitle("病例单",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("binglidan","新增"))|| Utils.isAuthFront("binglidan","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_BINGLIDAN).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_BINGLIDAN)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("binglidan",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_BINGLIDAN,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun BinglidancommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("jiuzhenshijian","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("jiuzhenshijian")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("binglidan", requestParams)
        }.otherwise {
                mListModel.list("binglidan", requestParams)
        }
    }
}