...
import com.design.appproject.ui.tijianbaogao.ListFilterDialog
...
import com.design.appproject.databinding.TijianbaogaocommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.tijianbaogao.ListModel
import com.design.appproject.ui.tijianbaogao.ListAdapter
 ...
 * 体检报告列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_TIJIANBAOGAO)
class ListFragment : BaseBindingFragment<TijianbaogaocommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("体检日期","tijianriqi")/*查询索引*/
    ...
    override fun initEvent() {
        setBarTitle("体检报告",mHasBack)
        setBarColor("#FFFFFF","black")
        mIsBack.no{
            params.put("sfsh","是")
        }
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("tijianbaogao","新增"))|| Utils.isAuthFront("tijianbaogao","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_TIJIANBAOGAO).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_TIJIANBAOGAO)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("tijianbaogao",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_TIJIANBAOGAO,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun TijianbaogaocommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("tijianriqi","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("tijianriqi")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("tijianbaogao", requestParams)
        }.otherwise {
                mListModel.list("tijianbaogao", requestParams)
        }
    }
}