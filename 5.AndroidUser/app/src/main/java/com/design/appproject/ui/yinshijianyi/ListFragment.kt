...
import com.design.appproject.ui.yinshijianyi.ListFilterDialog
...
import com.design.appproject.databinding.YinshijianyicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.yinshijianyi.ListModel
import com.design.appproject.ui.yinshijianyi.ListAdapter
 ...
 * 饮食建议列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_YINSHIJIANYI)
class ListFragment : BaseBindingFragment<YinshijianyicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("饮食建议",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("yinshijianyi","新增"))|| Utils.isAuthFront("yinshijianyi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_YINSHIJIANYI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("yinshijianyi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_YINSHIJIANYI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun YinshijianyicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("yinshijianyi", requestParams)
        }.otherwise {
                mListModel.list("yinshijianyi", requestParams)
        }
    }
}