...
import com.design.appproject.ui.jiankangjihua.ListFilterDialog
...
import com.design.appproject.databinding.JiankangjihuacommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.jiankangjihua.ListModel
import com.design.appproject.ui.jiankangjihua.ListAdapter
 ...
 * 健康计划列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_JIANKANGJIHUA)
class ListFragment : BaseBindingFragment<JiankangjihuacommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("日期","riqi")/*查询索引*/
    ...
    override fun initEvent() {
        setBarTitle("健康计划",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("jiankangjihua","新增"))|| Utils.isAuthFront("jiankangjihua","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_JIANKANGJIHUA).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_JIANKANGJIHUA)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("jiankangjihua",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_JIANKANGJIHUA,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun JiankangjihuacommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("riqi","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("riqi")
        }
        ...
        mIsBack.yes {
                mListModel.page("jiankangjihua", requestParams)
        }.otherwise {
                mListModel.list("jiankangjihua", requestParams)
        }
    }
}