...
import com.design.appproject.ui.jiankangzhishi.ListFilterDialog
...
import com.design.appproject.databinding.JiankangzhishicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.jiankangzhishi.ListModel
import com.design.appproject.ui.jiankangzhishi.ListAdapter
 ...
 * 健康知识列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_JIANKANGZHISHI)
class ListFragment : BaseBindingFragment<JiankangzhishicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("健康知识",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("jiankangzhishi","新增"))|| Utils.isAuthFront("jiankangzhishi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_JIANKANGZHISHI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_JIANKANGZHISHI)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("jiankangzhishi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_JIANKANGZHISHI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun JiankangzhishicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("jiankangzhishi", requestParams)
        }.otherwise {
                mListModel.list("jiankangzhishi", requestParams)
        }
    }
}