...
import com.design.appproject.databinding.JiankangjihuaFilterDialogLayoutBinding
...
class ListFilterDialog(context: Context) : DrawerPopupView(context) {
    ...
    lateinit var binding: JiankangjihuaFilterDialogLayoutBinding
    ...
    override fun onCreate() {
        ...
        binding.apply {
            ...
            resetBtn.setOnClickListener { //重置
                yonghuzhanghaoEt.setText("")
                callBackListener?.invoke(false,null)
            }
            ensureBtn.setOnClickListener { //确定
                params.clear()
                yonghuzhanghaoEt.text.toString().isNotNullOrEmpty().yes {
                    val filterContent = yonghuzhanghaoEt.text.toString()
                    params.put("yonghuzhanghao","%${filterContent}%")
                }
